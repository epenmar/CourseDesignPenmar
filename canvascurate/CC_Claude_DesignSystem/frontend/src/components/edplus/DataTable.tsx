...
export default function DataTable<TRow extends { id: string }>({
    ...
    return (
      <label className="inline-flex cursor-pointer" aria-label={label}>
        <input
          ...
          onChange={onChange}
          className="h-4 w-4 rounded border-outline accent-primary cursor-pointer"
        />
      </label>
    );
  }
  ...
  function RowActionsCell({ row }: { row: TRow }) {
    ...
    return (
      <td className="relative px-3 py-3 text-right" onClick={(e) => e.stopPropagation()}>
        <button
          ...
          className="inline-flex h-7 w-7 items-center justify-center rounded-md text-on-surface-variant hover:bg-surface-container hover:text-on-surface transition-colors"
        >
              ...
              {data.map((row, rowIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {rowActions && <RowActionsCell row={row} />}
                  </tr>
                );
              })}
            </tbody>
